...
import { Router, Share2, Trash2, CheckCircle2, AlertCircle, Moon } from 'lucide-react';
...
export default function RouterNode({ id, data, selected }) {
  ...
  const getBrandIcon = (label) => {
    return <span className="font-black text-blue-500 text-xs italic">MikroTik</span>;
  };

  return (
    <div 
      className={`relative w-36 bg-white/80 backdrop-blur-md rounded-xl border transition-all duration-300 
        ${selected ? 'border-red-500 shadow-xl ring-2 ring-red-500/20' : 'border-slate-200 shadow-sm hover:border-red-300 hover:shadow-md'}`}
    >
      ...
      <div className="flex flex-col items-center pt-8 pb-3 px-2 min-h-[110px]">
        {/* Ikon Perangkat */}
        <div className="w-10 h-10 bg-white shadow-sm border border-slate-100 rounded-xl flex items-center justify-center mb-2 text-red-500 transform transition-transform hover:scale-105">
          {getBrandIcon(data.label)}
        </div>
        
        {/* Info */}
        <div className="w-full flex flex-col items-center">
          <h3 className="font-bold text-slate-700 text-[11px] tracking-tight truncate w-full text-center">{data.label || `Router ${data.index || 1}`}</h3>
