...
import { Wifi, Settings, Trash2, CheckCircle2, XCircle, AlertTriangle, Moon } from 'lucide-react';
  ...
  return (
    <div 
      className={`relative w-28 bg-gradient-to-b from-white to-slate-50 rounded-xl border transition-all duration-300 
        ${selected ? 'border-amber-500 shadow-xl ring-2 ring-amber-50' : 'border-slate-200 shadow hover:shadow-lg hover:border-amber-300'}`}
    >
      {/* Header Actions */}
      <div className="absolute top-1 w-full px-1 flex justify-between items-center z-10 pointer-events-none">
        <div className="flex gap-1.5 pointer-events-auto">
          <button 
            ...
            className="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-md transition-all group"
      ...
      <div className="flex flex-col items-center pt-6 pb-2 px-2">
        {/* Ikon Perangkat */}
        <div className="w-8 h-8 bg-white shadow-sm border border-slate-100 rounded-lg flex items-center justify-center mb-1 text-amber-500 transform transition-transform hover:scale-105">
          <Wifi size={18} strokeWidth={1.5} />
        </div>
        
        {/* Info */}
        <div className="w-full border-t border-slate-100 pt-1.5 flex flex-col items-center">
          <h3 className="font-bold text-slate-700 text-[10px] tracking-tight truncate w-full text-center">{data.label || 'Modem'}</h3>
          <p className="text-[7px] text-slate-400 font-bold uppercase tracking-wider text-center mt-0.5">Bridge to WAN</p>
        </div>
      </div>
